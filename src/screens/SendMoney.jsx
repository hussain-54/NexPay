...
export const SendMoney = () => {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      refreshUserAccount();
      setStep(4);
      // Also expose dedicated success route for deep links / demos
      // navigate('/tx-success', { state: { amount: receivedAmount, currency: currency.toUpperCase(), signature: res.signature } });
    } catch (err) {
      ...
      // Optional dedicated failure screen for severe errors
      if (msg && !msg.includes("KycRequired") && !msg.includes("InsufficientFunds")) {
        // navigate('/tx-failed', { state: { message: msg } });
      }
    } finally {
