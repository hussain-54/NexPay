  ...
  persist(
    (set, get) => ({
      ...
      addNotification: (notif) => set((state) => ({
        notifications: [{ id: Date.now(), read: false, unread: true, time: new Date().toISOString(), ...notif }, ...state.notifications]
      })),
      getUnreadCount: () => get().notifications.filter(n => !n.read || n.unread).length,
